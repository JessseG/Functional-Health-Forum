  ...
  try {
    if (session) {
                  ...
                  <tr>
                    <td align="center" style="border-radius: 5px;" bgcolor="${backgroundColor}">
                      <div style="font-size: 14.5px; font-family: Helvetica, Arial, sans-serif; color: black; text-decoration: none; border-radius: 5px; 
                        padding: 7px 20px; border: 1px solid ${linkBorderColor}; display: inline-block; font-weight: 500; cursor: text;">${newPost.accessCode}
                      </div>
                    </td>
                    <td align="center">
                      <div style="margin-left:17px; border-radius: 5px; background-color:${buttonBackgroundColor};">
                        <a href="${process.env.NEXTAUTH_URL}/communities/${newPost.community.name}" target="_blank" style="font-size: 14.5px; font-family: Helvetica, Arial, 
      ...
      if (sendEmail) {
        const resp = await axios(sendEmail)
          .then(async function (response) {
            ...
            if (response.data) {
              ...
              return { status: response.status, newPost: newPost };
            }
        ...
        return res.json({ status: "Email failure", newPost: newPost });
      }
    }
  } catch (e) {
    // console.log(e);
